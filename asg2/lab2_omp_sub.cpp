...
// eUpdated has eTranspose now
	...
	// double sigSort[N];
	double *sigSort=(double*)malloc(sizeof(double*)*N);
	...
	// double eSorted[N][N];
	double **eSorted=(double**)malloc(sizeof(double*)*N);
	for(int i=0;i<N;i++) eSorted[i]=(double*)malloc(sizeof(double)*N);

	sort(sigSort,sigSort + N,greater<float>());
    ...
    for(int i=0;i<N;i++){
    	for(j=0;j<N;j++){
    		if(fabs(sigSort[i]-a[i][i] )<0.0001 && index[j]==true){
    			index[j]=false;
    			break;
    		}
    	}
    	for(int k=0;k<N;k++){
    		eSorted[k][i]=eUpdated[k][j];
    	}
    }

    for(int i=0;i<N;i++){
    	for(int j=0;j<N;j++){
    		eUpdated[i][j]=eSorted[i][j];
    	}
    }
...
void PCA(int retention, int M, int N, float* D, float* U, float* SIGMA, float** D_HAT, int *K)
{
 ...
 //    }
    ...
    for(int i=0;i<*K;i++){
    	for(j=0;j<N;j++){
    		if(fabs(sigmaSorted[i]-(SIGMA[j]*SIGMA[j]/sum) )<0.0001 && index[j]==true){
    			index[j]=false;
    			break;
    		}
    	}
